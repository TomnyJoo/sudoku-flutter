...
class StatisticsStorageService {
  // 缓存管理
  static final Map<String, GameStatistics> _cache = HashMap();
  static const int _maxCacheSize = 10;
  
  /// 添加到缓存
  static void _addToCache(String key, GameStatistics value) {
  ...
  /// 从缓存获取
  static GameStatistics? _getFromCache(String key) => _cache[key];
  ...
  /// 加载游戏统计
  static Future<GameStatistics?> loadStatistics(
    ...
    try {
      ...
      final statistics = GameStatistics.fromJson(jsonDecode(json), gameType);
    ...
    try {
      ...
      for (final gameType in gameTypeKeys) {
        try {
          ...
          if (statsData != null) {
            final stats = GameStatistics.fromJson(statsData, gameType);
            final key = getStorageKey(gameType);
  ...
  static Future<Map<String, GameStatistics>> getAllStatistics() async {
    ...
    final statisticsMap = <String, GameStatistics>{};
    for (final gameType in gameTypes) {
      final key = getStorageKey(gameType);
      final stats = await loadStatistics(gameType, key);
      statisticsMap[gameType] = stats ?? GameStatistics.empty(gameType);
    }
